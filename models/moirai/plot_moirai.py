...
def plot_real_vs_pred_series(real_df, pred_df, title, save_subdir):
    ...
    plt.plot(ts_pred, label="Previsão Moirai", color="tab:blue", linewidth=2)
    plt.title(title)
    plt.xlabel("Data")
...
def plot_initial_context_and_forecast(real_df, pred_df, df, window_type, save_subdir):
    ...
    plt.plot(pred_df.set_index(timestamp_col)["yhat"], label="Previsão Moirai", color="tab:blue", linewidth=2)
    plt.title(f"{window_type}: Contexto e previsão (Moirai)")
    plt.xlabel("Data")
    plt.ylabel(target_col)
    plt.legend()
    plt.grid(alpha=0.3)
    ...
    os.makedirs(save_subdir, exist_ok=True)
    plt.savefig(os.path.join(save_subdir, f"{window_type}_initial_context_moirai.png"), dpi=300)
    plt.show()
    plt.close()


# TODO: ajustar paths conforme estrutura do projeto
csv_path = "/home/luana/Desktop/TCC/inferencia-em-series-temporais-tcc/data/dataset.csv"
...
forecast_csvs = {
    "SW-1Y7D": "/home/luana/Desktop/TCC/inferencia-em-series-temporais-tcc/results/moirai/moirai_SW-1Y7D.csv",
    "SW-2Y7D": "/home/luana/Desktop/TCC/inferencia-em-series-temporais-tcc/results/moirai/moirai_SW-2Y7D.csv",
    "EW-7D":   "/home/luana/Desktop/TCC/inferencia-em-series-temporais-tcc/results/moirai/moirai_EW-7D.csv",
}

for label, csv in forecast_csvs.items():
    df_pred = pd.read_csv(csv, parse_dates=[timestamp_col])
    plot_real_vs_pred_series(real_2019, df_pred, f"{label} vs Real 2019 (Moirai)", save_dir)
