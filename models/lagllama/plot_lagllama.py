...
def plot_real_vs_pred_series(real_df, pred_df, title, save_subdir):
    ...
    plt.plot(ts_pred, label="Previsão Lag-Llama", color="tab:orange", linewidth=2)
    plt.title(title)
    plt.xlabel("Data")
...
def plot_initial_context_and_forecast(real_df, pred_df, df, window_type, save_dir):
    ...
    plt.plot(pred_df.set_index(timestamp_col)["yhat"], label="Previsão Lag-Llama", color="tab:orange", linewidth=2)
    plt.title(f"{window_type}: Contexto e previsão (Lag-Llama)")
    plt.xlabel("Data")
    plt.ylabel(target_col)
    plt.legend()
    plt.grid(alpha=0.3)
    ...
    os.makedirs(save_dir, exist_ok=True)
    plt.savefig(os.path.join(save_dir, f"{window_type}_initial_context_llama.png"), dpi=300)
    plt.show()
    plt.close()
...
forecast_csvs = {
    "SW-1Y7D": "/home/luana/Downloads/TCC/results/lag_llama/lagllama_SW-1Y7D.csv",
    "SW-2Y7D": "/home/luana/Downloads/TCC/results/lag_llama/lagllama_SW-2Y7D.csv",
    "EW-7D": "/home/luana/Downloads/TCC/results/lag_llama/lagllama_EW-7D.csv",
}

for label, csv in forecast_csvs.items():
    df_pred = pd.read_csv(csv, parse_dates=[timestamp_col])

    plot_real_vs_pred_series(real_2019, df_pred, f"{label} vs Real 2019 (Lag-Llama)", save_dir)
